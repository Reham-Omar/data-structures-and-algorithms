...
const validatePin = (pin) => {
    // Solution code here...
    let regularExpression = /^\d\d\d\d$/g, result;

    if (typeof pin === 'number') {
        pin = pin.toString();
    }
    let valid = pin.match(regularExpression);
    if (valid) {
        result = true;
    }
    else {
...
const validateEmail = (email) => {
    // Solution code here...
    let regularExpression = /^\w+(.\w+)@\w+\.(net|com|org)$/g, result;

    let valid = email.match(regularExpression);
    if (valid) {
        result = true;
    }
    else {
...
Acceptable formats include:
 ...
 - 555555-5555
 - 5555555555
Your function should include a single regular expression pattern that valid any of these formats.
Return either true or false.
------------------------------------------------------------------------------------------------ */

const validatePhoneNumber = (phoneNumber) => {
    // Solution code here...
    let regularExpression = /^(\d{3}|\(\d{3}\))\D?(\d{3})\D?(\d{4})$/g, result;

    let valid = phoneNumber.match(regularExpression);
    if (valid) {
        result = true;
    }
    else {
...
const findTagNames = elements => {
    ...
    elements.forEach(val => {
        let valid = val.match(regularExpression);
        valid.forEach(val => {
            array.push(val);
        });
    });
