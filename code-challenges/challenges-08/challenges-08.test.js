...
const filterStringsWithVowels = (arr) => {
    ...
    let words = arr.filter(val => {
        if (/[aeiou]/.test(val)) return val;
    });
    return words;
...
const getStatName = (arr, minBaseStat) => {
    // Solution code here...
     let nameobj=getBaseStatGreaterThan(arr, minBaseStat).map(val =>{
