...
const count = (target, input) => {
    ...
    let counter = 0;
    let num1 = input.map(value => {
        let num2 = value.map(num => {
            if (num === target) {
                return counter++;
            }
        })
...
const divisibleByFiveTwoToThePower = (input) => {
    ...
    return input.map((arrays) =>
        arrays.filter((value) =>
            (typeof value === 'number' && value % 5 === 0)).map(innerValue =>
                Math.pow(2, innerValue)))
...
let findMaleAndFemale = (data) => {
    // Solution code here...
    let names = data.filter(val => 
        val.gender === 'male' || val.gender === 'female').map(valName =>
             valName.name).join(' and ');
    return names;
};
